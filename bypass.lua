--[[
    ScriptHub Analyzer v2 - Final Edition 

    Made by cbyerseall (Terry Davis) on dc
    
    Discord: https://discord.gg/KQMzeSpNbY
    ----------------------------------------------------------------------------
    Features:
      * Passive hooking for loadstring, HttpGet, RequestAsync, require and loadfile.
      * The original script remains unaffected.
      * Categorized logging: "SYSTEM", "LOADSTRING", "LOADSTRING_FULL", "LOADSTRING_LINK",
        "AUTODECOMPILE", "DEBUG", "HTTP", "REQUEST", "MODULE", "LOADFILE", "DECOMPILE".
      * All log messages are stored in a "logs" array.
      * All detected URLs are stored in a separate "detectedLinks" array.
      * Every unique loadstring call (full code) is stored in a "loadstringCalls" array.
      * The final JSON is saved (pretty printed) to "ScriptHubAnalyzer_Log.json" with keys "logs", "links", and "loadstringCalls".
      * Auto-detects and validates URLs in loadstring code.
      * If a loadstring returns obfuscated code (e.g. starting with "return(function(" and containing "local N={"),
        it instantly triggers auto-decompilation and saves the output.
      * Auto-decompilation uses Dex’s decompile logic (from :contentReference[oaicite:0]{index=0}:contentReference[oaicite:1]{index=1}) without opening Dex manually.
      * All heavy operations run asynchronously so the game does not freeze.
      * Basic decompilation race-condition handling via a simple waiting mechanism.
]]
