...
--// StorageSetup
local storageFolder = ReplicatedStorage:FindFirstChild("CheckpointCache")
if not storageFolder then
	storageFolder = Instance.new("Folder")
	storageFolder.Name = "CheckpointCache"
...
local rev = "Checkpoint touch v0.6  "
--============ GUI ==================--
...
local function cloneCheckpoint(cpName)
	local cpFolder = workspace:WaitForChild("CheckPoint")
	local cp = cpFolder:WaitForChild(cpName)

	-- cek apakah sudah di-clone
	local existing = storageFolder:FindFirstChild(cpName)
	if existing then
		return existing
	end

	local clone = cp:Clone()
	clone.Parent = storageFolder
	logBox.Text = cpName.." berhasil di-clone ke ReplicatedStorage"
	return clone
end
